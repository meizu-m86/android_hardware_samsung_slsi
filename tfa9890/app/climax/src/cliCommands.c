...
extern void tfa98xxDumpParams(char *parmsFilename);
extern void tfaRun_ValidateParams(nxpTfa98xxParameters_t *tfaParams,
		int defaultprofile);
extern int lxScriboGetPin(int fd, int pin);
extern int tfaRun_NameToParam( char *fullname, nxpTfa98xxParamsType_t type, nxpTfa98xxParameters_t *parms);
extern void tfaRun_ShowParameters( nxpTfa98xxParameters_t *parms);
extern tfa98xx_DspRead( unsigned int offset, Tfa98xx_handle_t *handlesIn);
extern tfa98xx_DspWrite( unsigned int address, int value,
		Tfa98xx_handle_t *handlesIn);
extern int lxScriboGetRev(int fd, char *str, int max);
#ifndef WIN32
/************************
